...
class ConverterViewModel @Inject constructor(
    ...
    override fun processIntents(intents: Observable<ConverterIntent>) = intents.subscribe(intentsSubject)
    ...
    companion object {
        private val reducer = BiFunction { previousState: ConverterViewState, result: ConverterResult ->
            when (result) {
                is ConverterResult.LoadCurrencyExchangeRatesResult -> {
                    when (result) {
                        is ConverterResult.LoadCurrencyExchangeRatesResult.Success -> previousState.copy(
                                isLoading = false,
                                conversionResult = conversionResult(previousState.baseCurrencyValue, previousState.chosenCurrency, result.exchangeRates),
                                baseCurrency = result.baseCurrency,
                                descriptionText = descriptionText(result.baseCurrency, previousState.chosenCurrency, result.exchangeRates)
                        )
                        is ConverterResult.LoadCurrencyExchangeRatesResult.Failure -> previousState.toFailure(result.error)
                ...
                is ConverterResult.LoadCurrencyExchangeRatesWithChosenCurrencyResult -> {
                    when (result) {
                        is ConverterResult.LoadCurrencyExchangeRatesWithChosenCurrencyResult.Success -> previousState.copy(
                                isLoading = false,
                                conversionResult = conversionResult(previousState.baseCurrencyValue, result.chosenCurrency, result.exchangeRates),
                                baseCurrency = result.baseCurrency,
                                chosenCurrency = result.chosenCurrency,
                                descriptionText = descriptionText(result.baseCurrency, result.chosenCurrency, result.exchangeRates)
                        )
                        is ConverterResult.LoadCurrencyExchangeRatesWithChosenCurrencyResult.Failure -> previousState.toFailure(result.error)
                ...
                is ConverterResult.ChangeChosenCurrencyAndRecalculateResult -> {
                    when (result) {
                        is ConverterResult.ChangeChosenCurrencyAndRecalculateResult.Success -> previousState.copy(
                                isLoading = false,
                                conversionResult = conversionResult(previousState.baseCurrencyValue, result.newChosenCurrency, result.exchangeRates),
                                chosenCurrency = result.newChosenCurrency,
                                descriptionText = descriptionText(previousState.baseCurrency, result.newChosenCurrency, result.exchangeRates)
                        )
                        is ConverterResult.ChangeChosenCurrencyAndRecalculateResult.Failure -> previousState.toFailure(result.error)
                ...
                is ConverterResult.ChangeBaseCurrencyValueResult -> {
                    when (result) {
                        is ConverterResult.ChangeBaseCurrencyValueResult.Success -> previousState.copy(
                                isLoading = false,
                                conversionResult = conversionResult(result.newValue, previousState.chosenCurrency, result.exchangeRates),
                                baseCurrencyValue = result.newValue,
                                descriptionText = descriptionText(previousState.baseCurrency, previousState.chosenCurrency, result.exchangeRates)
                        )
                        is ConverterResult.ChangeBaseCurrencyValueResult.Failure -> previousState.toFailure(result.error)
        ...
        ): String = "1 $baseCurrency = ${exchangeRates.rates[chosenCurrency]} $chosenCurrency as of ${exchangeRates.date}"
